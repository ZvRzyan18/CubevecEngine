...
void __cve_collision_handle_resolve(CVE_Manifold2D *manifold) {
 ...
 if(manifold->a->components.is_resting && manifold->b->components.is_resting)
  return;
...
void __cve_collision_handle_impulse(CVE_Manifold2D *manifold) {
 ...
 CVE_Vec2f	relative_velocity[2], impulse, vec_j, inv_mass, apply_impulse,
  ra[2], rb[2], omega_vec_a, omega_vec_b, ra_perp[2], rb_perp[2], angular_velocity_a, angular_velocity_b,
...
/*
 CVE_Vec2f a_velocity, b_velocity;
 CVE_Float a_omega, b_omega;

 if(a->components.is_resting) {
 	CVE_ScalarToVector2f(a_velocity, 0);
 	a_omega = 0;
 } else {
 	a_velocity = a->components.velocity;
 	a_omega = a->components.omega;
 }
 if(b->components.is_resting) {
 	CVE_ScalarToVector2f(b_velocity, 0);
 	b_omega = 0;
 } else {
 	b_velocity = b->components.velocity;
 	b_omega = b->components.omega;
 }
*/

 for(i = 0; i < manifold->contact_size; i++) {
 
 	CVE_Sub2f(ra[i], a->components.centroid, manifold->contact[i]);
  CVE_Sub2f(rb[i], b->components.centroid, manifold->contact[i]);
 	 ra_perp[i].x = -ra[i].y;
 	 ra_perp[i].y = ra[i].x;
 	 rb_perp[i].x = -rb[i].y;
 	 rb_perp[i].y = rb[i].x;
  ...
  CVE_Mul2f(angular_velocity_a, ra_perp[i], omega_vec_a);
  CVE_Mul2f(angular_velocity_b, rb_perp[i], omega_vec_b);
  ...
  CVE_Sub2f(relative_velocity[i], relative_velocity_b, relative_velocity_a);
  
  
  CVE_Dot2f(dp[i], relative_velocity[i], manifold->direction);
 
  if(dp[i] > 0.0)
   continue;
 
  CVE_Dot2f(ra_perp_dp, ra_perp[i], manifold->direction);
  CVE_Dot2f(rb_perp_dp, rb_perp[i], manifold->direction);
  ...
  CVE_Cross2f(impulse_scalar, ra[i], impulse);
  a->components.omega -= impulse_scalar * a->components.inv_rotational_inertia;
  
  CVE_Cross2f(impulse_scalar, rb[i], impulse);
  b->components.omega += impulse_scalar * b->components.inv_rotational_inertia;
...
*/
  ...
  CVE_Sub2f(tangent, relative_velocity[i], normal_dp);
  ...
  CVE_Dot2f(j, relative_velocity[i], tangent);
  j = -j;
  ...
  if(j_pos <= (contact_impulse[i] * friction)) {
   ...
   contact_impulse[i] = -contact_impulse[i];
...
/*
#define SLEEP_ROT_THRESHOLD 0.188
#define SLEEP_POS_THRESHOLD 0.188

 if(
 ((fabsf(a->components.velocity.x) < SLEEP_POS_THRESHOLD) && (fabsf(a->components.velocity.y) < SLEEP_POS_THRESHOLD) && (fabsf(a->components.omega) < SLEEP_ROT_THRESHOLD))
 ) {
 	CVE_ScalarToVector2f(a->components.velocity, 0);
 	a->components.omega = 0;
 }
 
 if(
 ((fabsf(b->components.velocity.x) < SLEEP_POS_THRESHOLD) && (fabsf(b->components.velocity.y) < SLEEP_POS_THRESHOLD) && (fabsf(b->components.omega) < SLEEP_ROT_THRESHOLD))
 ) {
 	CVE_ScalarToVector2f(b->components.velocity, 0);
 	b->components.omega = 0;
 }
